...
def make_header_info(data,size):
    ...
    HeaderData[15] = Len_Temp
     ...
     # 1. 将 CodeData (整数列表) 转换为 bytes 对象
    # CodeData 是 [0x12, 0x34, ...]，bytes() 可以直接处理
    code_bytes = bytes(CodeData)
    # try:
    #     with open("code_data_raw.txt", "w") as f:
    #         # 将 bytes 对象转换为 16 进制字符串并写入
    #         # .hex() 方法会自动将 b'\x01\x02' 转换为 '0102'
    #         f.write(code_bytes.hex())
    #     print("Raw code data saved to code_data_raw.txt")
    # except IOError as e:
    #     print(f"Error writing code_data_raw.txt: {e}")
    # 2. 计算 SHA256
    ...
    HeaderData[BOOT2_HEAR_SIZE_BYTE-4] = Len_Temp
    Len_Temp = (Boot2HeaderCrc >> 8) & 0xff
    HeaderData[BOOT2_HEAR_SIZE_BYTE-3] = Len_Temp
    Len_Temp = (Boot2HeaderCrc >> 16) & 0xff
    HeaderData[BOOT2_HEAR_SIZE_BYTE-2] = Len_Temp
    Len_Temp = (Boot2HeaderCrc >> 24) & 0xff
    HeaderData[BOOT2_HEAR_SIZE_BYTE-1] = Len_Temp
    new_data = []
    new_data[0:BOOT2_HEAR_SIZE_BYTE] = HeaderData
    new_data[BOOT2_HEAR_SIZE_BYTE:size + BOOT2_HEAR_SIZE_BYTE] = CodeData
    return new_data

def make_firm_bin(file):
    # Read bin & record data
    binfilepath = file
    binfilepath = binfilepath + r'.bin\FIRMWARE_HEADER'
    project_name = os.path.basename(file)
    filename = f"{project_name}_firmware"

    try:
        try:
            os.remove(filename + '.bin')
            print(f'已有BIN文件已删除, 准备生成新的BIN文件: {filename}.bin')
        except FileNotFoundError:
            print(f'无BIN文件, 准备生成新的BIN文件: {filename}.bin')

        # 直接读取源文件数据
        with open(binfilepath, "rb") as source_file:
        ...
        Format_data = make_header_info(new_data, len(data))
        input = bytes(Format_data)

        # 直接写入.bin文件
        with open(filename + '.bin', 'wb') as f:
            f.write(input)

        print(f"生成的firmware bin文件名: {filename}.bin")
        print("固件头信息生成成功:")
        print(filename + " Flag = " + hex(Flag))
        print(filename + " Version = " + hex(Version))
        print(filename + " LoadAddr = " + hex(LoadAddr))
        print(filename + " BIN size = ", len(data))
        print(filename + " CRC32 = " + hex(Crc))

    except Exception as e:
        print(f"固件生成bin文件失败: {e}")

def make_boot2_bin(file):
    binfilepath = file + r'.bin\BOOT2_HEADER'
    project_name = os.path.basename(file)
    filename = f"{project_name}_boot2"
    target_size = 4 * 1024  # 4K bytes
    ...
    try:
        ...
        # 写入目标文件并填充至4K
        with open(filename + '.bin', "wb") as target_file:
            target_file.write(source_data)
            if len(source_data) < target_size:
                padding = b'\x00' * (target_size - len(source_data))
                target_file.write(padding)

        print(f"生成的boot2 bin文件名: {filename}.bin")
    except Exception as e:
        print(f"BOOT2生成bin文件失败: {e}")
...
def combine_bin_files(boot2_path, firmware_path, output_path):
    ...
    try:
        ...
        print(f"调试版本合并生成的bin: {output_path}")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    #print(sys.argv[1])
    make_boot2_bin(sys.argv[1])
    make_firm_bin(sys.argv[1])
    ...
    if len(sys.argv) > 2 and sys.argv[2].lower() == 'debug':
        is_debug = True
    if is_debug:
        combine_bin_files(f'{project_name}_boot2.bin', f'{project_name}_firmware.bin', f'{project_name}.bin')

...
#ifndef __SLC_HAL_INTC_H__
#define __SLC_HAL_INTC_H__
 ...
 */
#define SLC_HAL_ENABLE_PERIPHERAL_IRQ(irq_num, priority)    \
    do {                                                    \
        NVIC_SetPriority((irq_num), (priority));            \
        NVIC_EnableIRQ((irq_num));                          \
    } while(0)

#define SLC_HAL_DISABLE_PERIPHERAL_IRQ(irq_num)     \
    do {                                            \
        NVIC_DisableIRQ((irq_num));                 \
    } while(0)

#define SLC_HAL_ENABLE_GLOBAL_IRQ() \
    do {                            \
        __enable_irq();             \
    } while(0)

#define SLC_HAL_DISABLE_GLOBAL_IRQ()    \
    do {                                \
        __disable_irq();                \
    } while(0)

// 中断屏蔽设置，1为屏蔽所有中断，0为不屏蔽
#define SLC_HAL_SET_PRIMASK(state)  \
    do {                            \
        __set_PRIMASK((state));     \
    } while(0)

// 获取当前PRIMASK状态
#define SLC_HAL_GET_PRIMASK()   \
    __get_PRIMASK()

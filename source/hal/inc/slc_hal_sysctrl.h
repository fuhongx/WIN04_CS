...
#ifndef __SLC_HAL_SYSCTRL_H__
#define __SLC_HAL_SYSCTRL_H__
 ...
 * @retval 0 成功, 其他失败
 */
int slc_hal_sysctrl_system_clock_init(hal_sysctrl_src_e clk_src, hal_sysctrl_div_e clk_div);
 ...
 */
uint32_t slc_hal_sysctrl_get_system_clock(void);
 ...
 */
void slc_hal_sysctrl_peripheral_clk_enable(hal_peripheral_clk_e peripheral, bool enable);
 ...
 * @param peripheral 外设选择，参考 hal_peripheral_clk_e 枚举
 */
void slc_hal_sysctrl_peripheral_mod_reset(hal_peripheral_clk_e peripheral);
 ...
 */
uint8_t slc_hal_sysctrl_get_reset_src(void);

/**
 * @brief Get reset source, 获取复位源, 软件设置的复位源，区别于slc_hal_sysctrl_get_reset_src，后者为硬件复位源
 * 
 * @return uint8_t 参考 hal_rst_src_e 枚举
 */
uint8_t slc_hal_sysctrl_sw_get_reset_src(void);

void slc_hal_sysctrl_reset_phy(void);

void slc_hal_sysctrl_disable_ahb_apb(void);

#ifdef __cplusplus
}
#endif

#endif /* __SLC_HAL_SYSCTRL_H__ */

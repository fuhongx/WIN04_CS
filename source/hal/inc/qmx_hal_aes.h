...
#ifndef __QMX_HAL_AES_H__
#define __QMX_HAL_AES_H__
 ...
 * 
 * @note 与qmx_hal_aes_update和qmx_hal_aes_final配合使用，用于分包运算
 */
 ...
 * 
 * @param mode AES算法类型，参考 hal_aes_mode_e 枚举，目前仅支持CBC模式
 * @note 与qmx_hal_aes_update和qmx_hal_aes_init配合使用，用于分包运算
 */
 ...
 * 
 * @note 与qmx_hal_aes_init和qmx_hal_aes_final配合使用，用于分包运算
 */
int qmx_hal_aes_update(uint8_t *in, uint32_t inlen, uint8_t *out);

/**
 * @brief AES算法加密，单次调用，无需调用qmx_hal_aes_init
 * 
 * @param key   AES算法密钥，长度必须为16字节（128位）
 * @param iv    AES算法初始化向量，长度必须为16字节（128位）
...
/**
 * @brief AES算法解密，单次调用，无需调用qmx_hal_aes_init
 * 
 * @param key   AES算法密钥，长度必须为16字节（128位）
 * @param iv    AES算法初始化向量，长度必须为16字节（128位）
 * @param enc_data AES算法输出数据的指针，长度与输入数据相同，单位为字节，为16字节（128位）的整数倍

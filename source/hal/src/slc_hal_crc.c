...
stCrcHandle_t *slc_hal_crc_get_handle(hal_crc_type_e crc)
{
    switch (crc) {
        case HAL_CRC16:
            return CRC16;
...
{
    stCrcInit_t stInit;
    stCrcHandle_t *crc = slc_hal_crc_get_handle(init->crc_type);
...
void slc_hal_crc_deinit(hal_crc_type_e crc_type)
{
    ...
    slc_hal_sysctrl_peripheral_mod_reset(crc_clk);
    slc_hal_sysctrl_peripheral_clk_enable(crc_clk, false);
}

uint32_t slc_hal_crc_calculate(hal_crc_type_e crc_type, uint8_t *data, uint32_t len)
{
    stCrcHandle_t *crc = slc_hal_crc_get_handle(crc_type);

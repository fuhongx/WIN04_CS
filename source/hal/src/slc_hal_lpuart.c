...
int slc_hal_lpuart_init(hal_lpuart_id_e lpuart, hal_lpuart_init_t *config)
{
...
void slc_hal_lpuart_deinit(hal_lpuart_id_e lpuart)
{
    ...
    slc_hal_sysctrl_peripheral_clk_enable(HAL_CLK_LPUART, false);
}

int slc_hal_lpuart_send_data(hal_lpuart_id_e lpuart, uint8_t *data, uint32_t len)
{
...
int slc_hal_lpuart_receive_data(hal_lpuart_id_e lpuart, uint8_t *data, uint32_t *len)
{
    if (lpuart >= HAL_LPUART_MAX || data == NULL) {
        return -1;
    }
...
void slc_hal_lpuart_enable_irq(hal_lpuart_id_e lpuart, uint32_t irq_mask)
{
    if (lpuart >= HAL_LPUART_MAX) {
        return;
    }
...
void slc_hal_lpuart_disable_irq(hal_lpuart_id_e lpuart, uint32_t irq_mask)
{
    if (lpuart >= HAL_LPUART_MAX) {
        return;
    }
...
uint32_t slc_hal_lpuart_get_irq_status(hal_lpuart_id_e lpuart)
{
    if (lpuart >= HAL_LPUART_MAX) {
        return 0;
    }
...
void slc_hal_lpuart_clear_irq_status(hal_lpuart_id_e lpuart, uint32_t irq_sta)
{
    if (lpuart >= HAL_LPUART_MAX) {
        return;
    }
...
#if HAL_LPUART_DMA_SUPPORT
void slc_hal_lpuart_dma_config(hal_lpuart_id_e lpuart, hal_lpuart_dma_cfg_t *config)
{
    if (lpuart >= HAL_LPUART_MAX || config == NULL) {
        return;
    }
...
int slc_hal_lpuart_dma_send(hal_lpuart_id_e lpuart, uint8_t *data, uint32_t len)
{
    if (lpuart >= HAL_LPUART_MAX || data == NULL) {
        return -1;
    }
    ...
    slc_hal_dma_transfer_cfg((hal_dma_ch_e)g_lpuart_dma_txch[lpuart], &dma_cfg);
    slc_hal_dma_start((hal_dma_ch_e)g_lpuart_dma_txch[lpuart]);

    return 0;
}

int slc_hal_lpuart_dma_receive(hal_lpuart_id_e lpuart, uint8_t *data, uint32_t len)
{
    if (lpuart >= HAL_LPUART_MAX || data == NULL) {
        return -1;
    }
    ...
    slc_hal_dma_transfer_cfg((hal_dma_ch_e)g_lpuart_dma_rxch[lpuart], &dma_cfg);
    slc_hal_dma_start((hal_dma_ch_e)g_lpuart_dma_rxch[lpuart]);

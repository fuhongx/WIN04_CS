...
static int slc_check_calendar_bcd(hal_rtc_calendar_t *calendar)
{
    if (calendar == NULL) {
        return -1;
    }
...
void slc_hal_rtc_set_calendar(hal_rtc_calendar_t *calendar)
{
    if (slc_check_calendar_bcd(calendar) != 0) {
        return;
    }
...
void slc_hal_rtc_get_calendar(hal_rtc_calendar_t *calendar)
{
    stRtcTime_t rtc_time;
...
void slc_hal_rtc_stop(void)
{
    rom_hw_rtc_start_counter(false);
}

void slc_hal_rtc_start(void)
{
    rom_hw_rtc_start_counter(true);
}

void slc_hal_rtc_set_alarm(hal_rtc_alarm_id_e alarm_index, hal_rtc_alarm_msk_t *alarm, hal_rtc_calendar_t *calendar)
{
    if (slc_check_calendar_bcd(calendar) != 0) {
        return;
    }
...
void slc_hal_rtc_enable_alarm(hal_rtc_alarm_id_e alarm_index)
{
    rom_hw_rtc_enable_alarm_interrupt(alarm_index);
}

void slc_hal_rtc_disable_alarm(hal_rtc_alarm_id_e alarm_index)
{
    rom_hw_rtc_disable_alarm_interrupt(alarm_index);
}

uint32_t slc_hal_rtc_get_alarm_int_sta(void)
{
    uint8_t sta;
...
void slc_hal_rtc_clear_alarm_int_sta(uint32_t sta)
{

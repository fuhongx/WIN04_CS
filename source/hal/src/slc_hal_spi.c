...
stSpiHandle_t *slc_hal_spi_get_handle(hal_spi_id_e spi_id)
{
    switch (spi_id) {
        case HAL_SPI0:
            return SPI0;
...
static void slc_hal_spi_set_clock_polarity_phase(stSpiHandle_t *spi, hal_spi_polarity_phase_e polarity_phase)
{
...
void slc_hal_spi_init(hal_spi_id_e id, hal_spi_cfg_t *spi_cfg)
{
    unSpiInit_t spi_init = {0};
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    ...
    slc_hal_spi_set_clock_polarity_phase(spi, spi_cfg->polarity_phase);
...
void slc_hal_spi_deinit(hal_spi_id_e id)
{
    ...
    slc_hal_sysctrl_peripheral_mod_reset(spi_clk);
    slc_hal_sysctrl_peripheral_clk_enable(spi_clk, false);
}

void slc_hal_spi_set_sw_cs(hal_spi_id_e id, hal_spi_cs_level_e cs_level)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return;
...
int slc_hal_spi_master_trx(hal_spi_id_e id, void *tx_buf, void *rx_buf, uint32_t len, uint32_t timeout)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return -1;
...
int slc_hal_spi_master_transmit(hal_spi_id_e id, void *tx_buf, uint32_t len, uint32_t timeout)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
...
int slc_hal_spi_master_receive(hal_spi_id_e id, void *rx_buf, uint32_t len, uint32_t timeout)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
...
int slc_hal_spi_slave_transmit(hal_spi_id_e id, void *tx_buf, uint32_t len, uint32_t timeout)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if ((spi == NULL) || (tx_buf == NULL) || (len == 0)) {
        return -1;
...
int slc_hal_spi_slave_receive(hal_spi_id_e id, void *rx_buf, uint32_t *len, uint32_t timeout)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if ((spi == NULL) || (rx_buf == NULL) || (*len == 0)) {
        return -1;
...
uint32_t slc_hal_spi_get_cur_sta(hal_spi_id_e id)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    uint32_t status = 0;
    if (spi == NULL) {
...
void slc_hal_spi_set_irq_mask(hal_spi_id_e id, uint32_t irq_mask)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return;
...
void slc_hal_spi_irq_enable(hal_spi_id_e id, uint32_t irq_mask)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return;
...
void slc_hal_spi_irq_disable(hal_spi_id_e id, uint32_t irq_mask)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return;
...
uint32_t slc_hal_spi_get_irq_sta(hal_spi_id_e id)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    uint32_t status = 0;
    if (spi == NULL) {
...
void slc_hal_spi_clear_irq_sta(hal_spi_id_e id, uint32_t irq_sta)
{
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
        return;
...
void slc_hal_spi_dma_config(hal_spi_id_e id, hal_spi_dma_cfg_t *spi_dma_init)
{
    ...
    slc_hal_dma_init(spi_dma_init->dma_ch, &dma_config);
}

int slc_hal_spi_dma_send(hal_spi_id_e id, void *data, uint32_t len)
{
    ...
    slc_hal_dma_transfer_cfg((hal_dma_ch_e)g_spi_dma_txch[id], &dma_cfg);
    slc_hal_dma_start((hal_dma_ch_e)g_spi_dma_txch[id]);

    return 0;
}

int slc_hal_spi_dma_recv(hal_spi_id_e id, void *data, uint32_t len)
{
    hal_dma_cfg_t dma_cfg;
    stSpiHandle_t *spi = slc_hal_spi_get_handle(id);
    if (spi == NULL) {
    ...
    slc_hal_dma_transfer_cfg((hal_dma_ch_e)g_spi_dma_rxch[id], &dma_cfg);
    slc_hal_dma_start((hal_dma_ch_e)g_spi_dma_rxch[id]);

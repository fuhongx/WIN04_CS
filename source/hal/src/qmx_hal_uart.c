...
stUartHandle_t *qmx_hal_uart_get_handle(hal_uart_id_e uart)
{
    switch (uart) {
        case HAL_UART0:
            return UART0;
...
{
    stUartInit_t Init = {0};
    stUartHandle_t *uartHandle = qmx_hal_uart_get_handle(uart);

    if ((config->baudrate > (qmx_hal_sysctrl_get_system_clock() / 16)) || (uartHandle == NULL))
        return -1;
...
void qmx_hal_uart_deinit(hal_uart_id_e uart)
{
    ...
    qmx_hal_sysctrl_peripheral_mod_reset(uart_clk);
    qmx_hal_sysctrl_peripheral_clk_enable(uart_clk, false);
}
...
int qmx_hal_uart_dma_send(hal_uart_id_e uart, uint8_t *data, uint32_t len)
{
    ...
    qmx_hal_dma_transfer_cfg((hal_dma_ch_e)g_uart_dma_txch[uart], &dma_cfg);
    qmx_hal_dma_start((hal_dma_ch_e)g_uart_dma_txch[uart]);
...
int qmx_hal_uart_dma_receive(hal_uart_id_e uart, uint8_t *data, uint32_t len)
{
    ...
    qmx_hal_dma_transfer_cfg((hal_dma_ch_e)g_uart_dma_rxch[uart], &dma_cfg);
    qmx_hal_dma_start((hal_dma_ch_e)g_uart_dma_rxch[uart]);

...
void DMA_IRQ_Handler(void)
{
    ...
    flag = qmx_hal_dma_get_interrupt_status();
    qmx_hal_dma_clear_interrupt_status(flag);

    PRINTF("DMA IRQ, flag=0x%X\n", flag);

    for (int i = 0; i < HAL_DMA_CH_MAX; i++) {
        if (flag & (1 << i)) {
            fin_num = qmx_hal_dma_get_finish_size((hal_dma_ch_e)i);
...
int qmx_dma_mem_to_mem_common_test(uint32_t src_addr, uint32_t dst_addr)
{
    int ret = 0;
    ...
    qmx_hal_sysctrl_peripheral_clk_enable(HAL_CLK_DMA, true);
    qmx_hal_sysctrl_peripheral_mod_reset(HAL_CLK_DMA);
...
start:
    ...
    qmx_hal_dma_init(chx, &dma_init);
    qmx_hal_register_irq_handler(DMA_IRQ, DMA_IRQ_Handler);
    QMX_HAL_ENABLE_PERIPHERAL_IRQ(DMA_IRQ, 0x3);
    qmx_hal_dma_start(chx);
...
int qmx_dma_ram_to_ram_test(void)
{
    ...
    if (qmx_dma_mem_to_mem_common_test((uint32_t)src_buf, (uint32_t)dst_buf) != 0) {
...
{
    int ret = 0;
    ret = qmx_dma_mem_to_mem_common_test(DMA_TEST_FLASH_SRC_ADDR, DMA_TEST_FLASH_DST_ADDR);
    if (ret != 0) {
...
int qmx_dma_ram_to_flash_test(void)
{
    ...
    ret = qmx_dma_mem_to_mem_common_test((uint32_t)buf, DMA_TEST_FLASH_DST_ADDR);
    if (ret != 0) {
    ...
    ret = qmx_dma_mem_to_mem_common_test(DMA_TEST_FLASH_DST_ADDR, (uint32_t)buf);
    if (ret != 0) {
...
int qmx_dma_diff_width_common_test(hal_dma_width_e width, bool muti_trig, uint16_t package_size, uint16_t trig_num)
{
    int ret = 0;
    hal_dma_ch_e chx = HAL_DMA_CH0;
    hal_dma_init_t dma_init;
...
start:
    ...
    while (g_dma_test_flag == 0) {
        if (muti_trig) {
            qmx_hal_dma_start(chx);
            test_trig_num++;
            qmx_hal_nop_delay_us(10);   // 确保有足够时间让DMA传输完成
        }
    }
...
{
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_BYTE, false, DMA_TEST_SIZE, 1) != 0) {
        PRINTF("====>DMA byte test fail\n");
        return -1;
    } else {
        PRINTF("====>DMA byte test success\n");
    }
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_HALFWORD, false, DMA_TEST_SIZE, 1) != 0) {
        PRINTF("====>DMA half word test fail\n");
        return -1;
    } else {
        PRINTF("====>DMA half word test success\n");
    }
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_WORD, false, DMA_TEST_SIZE, 1) != 0) {
...
{
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_BYTE, true, 1, DMA_TEST_SIZE) != 0) {
        PRINTF("====>DMA muti trig byte test fail\n");
        return -1;
    } else {
        PRINTF("====>DMA muti trig byte test success\n");
    }
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_HALFWORD, true, 2, DMA_TEST_SIZE/2) != 0) {
        PRINTF("====>DMA muti trig half word test fail\n");
        return -1;
    } else {
        PRINTF("====>DMA muti trig half word test success\n");
    }
    if (qmx_dma_diff_width_common_test(HAL_DMA_WIDTH_WORD, true, 4, DMA_TEST_SIZE/4) != 0) {
...
void qmx_dma_performance_test_irq(void)
{
    uint32_t flag = 0;

    g_dma_test_end_time = qmx_hal_timer_get_count(HAL_TIMER0);
...
int qmx_dma_performance_test(void)
{
    ...
    qmx_hal_register_irq_handler(DMA_IRQ, qmx_dma_performance_test_irq);
    QMX_HAL_ENABLE_PERIPHERAL_IRQ(DMA_IRQ, 0x3);
...
start:
    ...
    g_dma_test_start_time = qmx_hal_timer_get_count(HAL_TIMER0);
    qmx_hal_dma_start(chx);
...
void qmx_dma_uart_test_irq(void)
{
    uint32_t flag = 0;
    uint16_t fin_num = 0;
...
int qmx_dma_uart_trx_test(void)
{
    ...
    QMX_HAL_DISABLE_PERIPHERAL_IRQ(UART1_IRQ);
    qmx_hal_unregister_irq_handler(UART1_IRQ);

    qmx_hal_gpio_set_iomux(DMA_UART_TEST_TX_PIN, HAL_IOMUX_MODE0);
    qmx_hal_gpio_set_iomux(DMA_UART_TEST_RX_PIN, HAL_IOMUX_MODE0);
    ...
    qmx_hal_uart_init(DMA_UART_TEST_UART, &config);
    ...
    qmx_hal_uart_dma_config(DMA_UART_TEST_UART, &dma_cfg);
    ...
    qmx_hal_register_irq_handler(DMA_IRQ, qmx_dma_uart_test_irq);
    QMX_HAL_ENABLE_PERIPHERAL_IRQ(DMA_IRQ, 0x3);

    qmx_hal_uart_dma_receive(DMA_UART_TEST_UART, rx_buf, DMA_UART_TEST_SIZE);
    qmx_hal_uart_dma_send(DMA_UART_TEST_UART, tx_buf, DMA_UART_TEST_SIZE);
...
int qmx_dma_lpuart_trx_test(void)
{
    ...
    qmx_hal_lpuart_dma_config(HAL_LPUART0, &dma_cfg);
    ...
    qmx_hal_lpuart_dma_receive(HAL_LPUART0, rx_buf, DMA_UART_TEST_SIZE);
    qmx_hal_lpuart_dma_send(HAL_LPUART0, tx_buf, DMA_UART_TEST_SIZE);
...
int qmx_dma_spi_master_test(void)
{
    ...
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN4, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN5, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN6, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN7, HAL_IOMUX_MODE2);
    ...
    qmx_hal_sysctrl_peripheral_clk_enable(HAL_CLK_DMA, true);
    qmx_hal_sysctrl_peripheral_mod_reset(HAL_CLK_DMA);
    qmx_hal_spi_dma_config(HAL_SPI1, &dma_cfg);
    ...
    qmx_hal_spi_dma_recv(HAL_SPI1, rx_buf, DMA_UART_TEST_SIZE);
    qmx_hal_spi_dma_send(HAL_SPI1, tx_buf, DMA_UART_TEST_SIZE);
    ...
    qmx_hal_dma_deinit(rx_chx);
    qmx_hal_dma_deinit(tx_chx);
    NVIC_DisableIRQ(DMA_IRQ);
...
void qmx_iic_slave_dma_test_irq(void)
{
    uint32_t iic_int_flag;
    uint8_t test_buf[DMA_UART_TEST_SIZE] = {0};

    iic_int_flag = qmx_hal_iic_get_irq_status(HAL_IIC0);
    qmx_hal_iic_clear_irq_status(HAL_IIC0, iic_int_flag);

    if (iic_int_flag & HAL_IIC_IRQ_STA_READ_REQ) {
        qmx_hal_iic_slave_dma_send(HAL_IIC0, g_dma_test_iic_buf, DMA_UART_TEST_SIZE);
    } else if (iic_int_flag & HAL_IIC_IRQ_STA_RX_FIFO_FULL) {
        qmx_hal_iic_slave_dma_recv(HAL_IIC0, test_buf, DMA_UART_TEST_SIZE);
...
int qmx_dma_iic_slave_test(void)
{
    ...
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN4, HAL_IOMUX_MODE5);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN5, HAL_IOMUX_MODE5);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN4, HAL_GPIO_OPEN_DRAIN);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN5, HAL_GPIO_OPEN_DRAIN);
    ...
    qmx_hal_iic_init(HAL_IIC0, &config);
    qmx_hal_iic_irq_enable(HAL_IIC0, HAL_IIC_IRQ_RX_FIFO_FULL | HAL_IIC_IRQ_READ_REQ);
    qmx_hal_register_irq_handler(I2C0_IRQ, qmx_iic_slave_dma_test_irq);
    ...
    qmx_hal_iic_dma_config(HAL_IIC0, &dma_init);
...
int qmx_dma_iic_master_test(void)
{
    ...
    qmx_hal_iic_master_dma_send(HAL_IIC0, test_tx_buf, DMA_UART_TEST_SIZE);

    while (g_dma_test_flag == 0);

    qmx_hal_iic_master_dma_recv(HAL_IIC0, test_tx_buf, test_rx_buf, DMA_UART_TEST_SIZE);

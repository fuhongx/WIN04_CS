...
int slc_crc16_accuracy_test(void)
{
    ...
    hw_crc = slc_hal_crc_calc16(crc_data, sizeof(crc_data));
...
int slc_crc24_accuracy_test(void)
{
    ...
    hw_crc = slc_hal_crc_calc24(crc_data, sizeof(crc_data));
...
int slc_crc32_accuracy_test(void)
{
    uint32_t hw_crc, sw_crc;
    uint8_t crc_data[8] = {0};
    int ret;
    ...
    hw_crc = slc_hal_crc_calc32(crc_data, sizeof(crc_data));
...
#define SLC_CRC_PERF_TEST_SIZE  1024
int slc_crc_performance_test(void)
{
    uint32_t start_time = 0;
    uint32_t end_time = 0;
    uint32_t crc_value __MAYBE_UNUSED, ret;
    uint8_t *data = malloc(SLC_CRC_PERF_TEST_SIZE);
    hal_rng_init_t rng_init = {0};
    ...
    rng_init.seed = 0xA4567891;
    slc_hal_rng_init(&rng_init);
    ret = slc_hal_get_random_u8(data, SLC_CRC_PERF_TEST_SIZE, 1000);
    if (ret != 0) {
        PRINTF("get random u8 error, ret=0x%X\n", ret);
        return ret;
    }
    ...
    crc_value = slc_hal_crc_calc16(data, SLC_CRC_PERF_TEST_SIZE);
    end_time = slc_hal_timer_get_count(HAL_TIMER0);
    PRINTF("CRC16 CCITT performance test, size: %u byte, cost time: %.2f us, perf: %.2f Mbps\n",
           SLC_CRC_PERF_TEST_SIZE, (float)(start_time - end_time) / 50.0f,
           SLC_CRC_PERF_TEST_SIZE * 8 * 50.0f / (1.024f * 1.024f) / (start_time - end_time));

    start_time = slc_hal_timer_get_count(HAL_TIMER0);
    crc_value = slc_hal_crc_calc24(data, SLC_CRC_PERF_TEST_SIZE);
    end_time = slc_hal_timer_get_count(HAL_TIMER0);
    ...
    start_time = slc_hal_timer_get_count(HAL_TIMER0);
    crc_value = slc_hal_crc_calc32(data, SLC_CRC_PERF_TEST_SIZE);
    end_time = slc_hal_timer_get_count(HAL_TIMER0);
...
once:
    ...
    cal_len = (cal_len % SLC_CRC_PERF_TEST_SIZE == 0) ? SLC_CRC_PERF_TEST_SIZE : cal_len % SLC_CRC_PERF_TEST_SIZE;
    ...
    switch (crc_type) {
        case HAL_CRC16:
            ...
            hw_crc = slc_hal_crc_calculate(HAL_CRC16, data, cal_len) & 0xFFFF;
        ...
        case HAL_CRC24:
            ...
            slc_hal_crc_init(&crc_init);
            hw_crc = slc_hal_crc_calculate(HAL_CRC24, data, cal_len) & 0xFFFFFF;
        ...
        case HAL_CRC32:
            ...
            slc_hal_crc_init(&crc_init);
            hw_crc = slc_hal_crc_calculate(HAL_CRC32, data, cal_len);

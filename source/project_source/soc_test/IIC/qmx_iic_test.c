...
void qmx_iic_test_init(void)
{
    ...
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN6, HAL_GPIO_OPEN_DRAIN);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN7, HAL_GPIO_OPEN_DRAIN);
}
...
void qmx_iic_test_deinit(void)
{
    ...
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN4, HAL_IOMUX_GPIO);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN5, HAL_IOMUX_GPIO);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN5, HAL_GPIO_PULL_NONE);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN4, HAL_GPIO_PULL_NONE);

    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN6, HAL_IOMUX_GPIO);
    qmx_hal_gpio_set_iomux(HAL_GPIO_PIN7, HAL_IOMUX_GPIO);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN6, HAL_GPIO_PULL_NONE);
    qmx_hal_gpio_set_mode(HAL_GPIO_PIN7, HAL_GPIO_PULL_NONE);
}

int qmx_iic_test_master_trx_common(hal_iic_id_e iic_id)
{
    uint8_t tx_data[QMX_IIC_TEST_TRX_LEN] = {0};
    uint8_t rx_data[QMX_IIC_TEST_TRX_LEN] = {0};
    uint32_t rx_len = 0;
    int ret = 0;

    ret = qmx_hal_get_random_u8(tx_data, QMX_IIC_TEST_TRX_LEN, 1000);
    if (ret != 0) {
    ...
    qmx_hal_iic_master_send(iic_id, tx_data, QMX_IIC_TEST_TRX_LEN, HAL_IIC_TIMEOUT);

    // 2、确保slave端处理完数据
    qmx_hal_nop_delay_ms(1000);

    // 3、master再接收数据, slave端在中断内将接收数据原路返回
    rx_len = QMX_IIC_TEST_TRX_LEN;
    ret = qmx_hal_iic_master_recv(iic_id, rx_data, &rx_len, HAL_IIC_TIMEOUT);

    // 4、比较接收长度和数据是否一致
    if ((ret != 0) && (rx_len != QMX_IIC_TEST_TRX_LEN)) {
        PRINTF("IIC%d master trx data len: %u != %u\n", iic_id, rx_len, QMX_IIC_TEST_TRX_LEN);
        if (rx_len != 0) {
    ...
    if (memcmp(rx_data, tx_data, QMX_IIC_TEST_TRX_LEN) != 0) {
        PRINTF("IIC%d master trx data is different.\n", iic_id);
        dump_u8buf("tx", tx_data, QMX_IIC_TEST_TRX_LEN);
        dump_u8buf("rx", rx_data, QMX_IIC_TEST_TRX_LEN);
        return -1;
    }
...
int qmx_iic_master_speed_test(void)
{
    ...
    config.addr = QMX_TEST_I2C_SLAVE_ADDR_7BIT;
...
start:
    qmx_test_master_send_cmd(QMX_TEST_CMD_I2C_CFG, tx_data, 8);
    ...
    for (i = 0; i <= HAL_IIC_SPEED_1M; i++) {
        ...
        qmx_hal_iic_init((hal_iic_id_e)tx_data[7], &config);

        ret = qmx_iic_test_master_trx_common((hal_iic_id_e)tx_data[7]);
        if (ret != 0) {
...
start:
    for (i = 0; i <= HAL_IIC_ADDR_10BIT; i++) {
        ...
        if (config.addr_bits == HAL_IIC_ADDR_10BIT) {
            config.addr = QMX_TEST_I2C_SLAVE_ADDR_10BIT;
        } else {
            config.addr = QMX_TEST_I2C_SLAVE_ADDR_7BIT;
        }
        tx_data[3] = config.addr & 0xFF;
...
uint8_t g_iic_test_slv_tx_data[QMX_IIC_TEST_TRX_LEN] = {0};
uint8_t g_iic_test_slv_rx_data[QMX_IIC_TEST_TRX_LEN] = {0};
volatile uint32_t g_iic_test_slv_rx_len = 0;
volatile int g_iic_test_slv_id = HAL_IIC0;

void qmx_iic_test_slv_irq(void)
{
    uint32_t iic_int_flag;
    int ret = 0;

    iic_int_flag = qmx_hal_iic_get_irq_status(g_iic_test_slv_id);
    qmx_hal_iic_clear_irq_status(g_iic_test_slv_id, iic_int_flag);

    if (iic_int_flag & HAL_IIC_IRQ_STA_READ_REQ) {
        ret = qmx_hal_iic_slave_send(g_iic_test_slv_id, g_iic_test_slv_tx_data, QMX_IIC_TEST_TRX_LEN, HAL_IIC_TIMEOUT);
        if (ret != 0) {
            PRINTF("IIC%d slave tx error, ret=0x%X\n", g_iic_test_slv_id, ret);
        }
    } else if (iic_int_flag & HAL_IIC_IRQ_STA_RX_FIFO_FULL) {
        g_iic_test_slv_rx_len = QMX_IIC_TEST_TRX_LEN;
        ret = qmx_hal_iic_slave_recv(g_iic_test_slv_id, g_iic_test_slv_rx_data, (uint32_t *)&g_iic_test_slv_rx_len, HAL_IIC_TIMEOUT);
...
int qmx_iic_test_slv_trx_common(hal_iic_id_e iic_id)
{
    uint8_t tx_data[QMX_TEST_RX_MAX_LEN] = {0};
    uint8_t rx_data[QMX_TEST_RX_MAX_LEN] = {0};
    uint8_t rx_len = 0;
    ...
    memset(g_iic_test_slv_tx_data, 0, QMX_IIC_TEST_TRX_LEN);
    memset(g_iic_test_slv_rx_data, 0, QMX_IIC_TEST_TRX_LEN);
    g_iic_test_slv_rx_len = 0;
    ret = qmx_hal_get_random_u8(g_iic_test_slv_tx_data, QMX_IIC_TEST_TRX_LEN, 1000);
    if (ret != 0) {
        PRINTF("get random u8 error, ret=0x%X\n", ret);
        return ret;
    }
    ...
    qmx_test_master_send_cmd(QMX_TEST_CMD_I2C_S_TRX, tx_data, 2);
    ret = qmx_test_master_get_result(rx_data, &rx_len);
    if ((ret != 0) || (rx_data[0] != 0)) {
    ...
    // 4、比较接收长度和数据是否一致
    if ((g_iic_test_slv_rx_len != QMX_IIC_TEST_TRX_LEN)) {
        PRINTF("IIC%d slv trx data len: %u != %u\n", iic_id, g_iic_test_slv_rx_len, QMX_IIC_TEST_TRX_LEN);
        if (g_iic_test_slv_rx_len != 0) {
            dump_u8buf("tx", g_iic_test_slv_tx_data, QMX_IIC_TEST_TRX_LEN);
            dump_u8buf("rx", g_iic_test_slv_rx_data, QMX_IIC_TEST_TRX_LEN);
        }
        return -1;
    }

    if (memcmp(g_iic_test_slv_tx_data, g_iic_test_slv_rx_data, QMX_IIC_TEST_TRX_LEN) != 0) {
...
start:
    ...
    for (i = 0; i <= HAL_IIC_ADDR_10BIT; i++) {
        ...
        if (g_iic_test_slv_id == HAL_IIC0) {
            ...
            qmx_hal_register_irq_handler(I2C0_IRQ, qmx_iic_test_slv_irq);
            QMX_HAL_ENABLE_PERIPHERAL_IRQ(I2C0_IRQ, 0x3);
        } else {
            qmx_hal_sysctrl_peripheral_clk_enable(HAL_CLK_I2C1, true);
            qmx_hal_sysctrl_peripheral_mod_reset(HAL_CLK_I2C1);

            qmx_hal_register_irq_handler(I2C1_IRQ, qmx_iic_test_slv_irq);
            QMX_HAL_ENABLE_PERIPHERAL_IRQ(I2C1_IRQ, 0x3);
        }

        qmx_hal_iic_init((hal_iic_id_e)g_iic_test_slv_id, &config);
        qmx_hal_iic_irq_enable((hal_iic_id_e)g_iic_test_slv_id, HAL_IIC_IRQ_RX_FIFO_FULL | HAL_IIC_IRQ_READ_REQ);

        ret = qmx_iic_test_slv_trx_common((hal_iic_id_e)g_iic_test_slv_id);
        if (ret != 0) {

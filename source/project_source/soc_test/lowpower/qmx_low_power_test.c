...
extern void qmx_platform_init(void);
...
void LPUART_IRQ_Handler(void)
{
    ...
    test_recv_len = 16;
    qmx_hal_lpuart_receive_data(HAL_LPUART0, test_recv_data, &test_recv_len);
...
#ifdef NT_SHELL
    extern void qmx_debug_uart_irq_handler(void);
...
__RETENTION_FUNC void after_wakeup_standby(void)
{
    // 睡前拉低，醒后拉高，统计唤醒时间
    qmx_hal_sysctrl_peripheral_clk_enable(HAL_CLK_GPIO, true);
    qmx_hal_gpio_set_iomux(TEST_GPIO_PIN, HAL_IOMUX_GPIO);
    qmx_hal_gpio_set_direction(TEST_GPIO_PIN, HAL_GPIO_OUTPUT);
    qmx_hal_gpio_set_value(TEST_GPIO_PIN, HAL_GPIO_LEVEL_LOW);
...
void wakeup_by_lptimer(hal_lptimer_id_e lptimer_id)
{
    ...
    qmx_hal_sysctrl_peripheral_clk_enable(lptimer_gate, true);
    qmx_hal_sysctrl_peripheral_mod_reset(lptimer_gate);

    qmx_hal_lptimer_init(lptimer_id, HAL_SYS_CLK_FREQ_32K * 2, true);

    if (lptimer_id == HAL_LPTIMER0)
        qmx_hal_register_irq_handler(LPTIMER0_IRQ, LPTIMER0_IRQ_Handler);
    else
        qmx_hal_register_irq_handler(LPTIMER1_IRQ, LPTIMER1_IRQ_Handler);

    QMX_HAL_ENABLE_PERIPHERAL_IRQ((IRQn_Type)lptimer_irq, 0x3);

    qmx_hal_lptimer_start(lptimer_id);
...
void wakeup_by_lpuart(void)
{
    ...
    qmx_hal_register_irq_handler(LPUART_IRQ, LPUART_IRQ_Handler);
...
void wakeup_by_rtc(void)
{
    ...
    qmx_hal_register_irq_handler(RTC_IRQ, RTC_IRQ_Handler);
    QMX_HAL_ENABLE_PERIPHERAL_IRQ(RTC_IRQ, 0x3);

    qmx_hal_rtc_start();
}

void qmx_lpwr_test_lpio_irq(void)
{
...
{
    qmx_hal_register_irq_handler(LPIO_IRQ, qmx_lpwr_test_lpio_irq);
    ...
    qmx_hal_lpio_set_direction(pin, HAL_GPIO_INPUT);

    qmx_hal_lpio_set_interrupt(pin, HAL_GPIO_INTR_LOW_LEVEL);
    qmx_hal_lpio_enable_interrupt(pin, true);
    qmx_hal_lpio_clear_interrupt();
...
void wakeup_src_select(uint32_t lp_wakeup_src_msk)
{
    ...
    qmx_hal_pmu_set_lp_wakeup_source(lp_wakeup_src_msk);
}
...
int low_power_sleep_phy_disable_test(void)
{
    ...
    qmx_lpwr_ctrl_init(&cfg);

    QMX_HAL_ENABLE_GLOBAL_IRQ();

    qmx_lpwr_ctrl_sleep(HAL_PMU_LP_MODE_SLEEP);
...
int low_power_stop_phy_disable_test(void)
{
    ...
    qmx_lpwr_ctrl_sleep(HAL_PMU_LP_MODE_STOP);
...
int low_power_standby_wakeup_by_lpio_test(void)
{
    ...
    qmx_lpwr_ctrl_sleep(HAL_PMU_LP_MODE_STANDBY);
...
int low_power_shutdown_test(void)
{
    ...
    qmx_lpwr_ctrl_sleep(HAL_PMU_LP_MODE_SHUTDOWN);

...
void qmx_phy_display_rx_data(void)
{
    uint32_t rx_sta = qmx_phy_monitor_get_rx_det_sta();
    PRINTF("PHY RX_DET_HEADER: 0x%X\n", rx_sta);
    qmx_phy_read_rx_buffer(phy_test_rx_buffer, (rx_sta & PHY_RX_PAYLOAD_LEN_MASK) >> PHY_RX_PAYLOAD_LEN_SHIFT);
...
static qmx_phy_test_pkg_t g_phy_test_pkg = {0};
...
void PHY0_IRQ_Handler(void)
{
    ...
    if (irq_flag & PHY_IRQ_SFSEARCH_FAIL_MASK) {
        ...
        qmx_sf_search_training(sf_train_val[sf_fail_cnt % 256]);
        // if ((sf_fail_cnt % 20) == 0)
...
void qmx_rffe_irq_handler(void)
{
    uint32_t irq_flag = qmx_rf_irq_get_sta();

    // clear irq flag
    qmx_rf_irq_clr_sta(irq_flag);
...
void qmx_phy_test(void)
{
    ...
    memset(&g_phy_test_pkg, 0, sizeof(qmx_phy_test_pkg_t));

    hal_rng_init_t rng_init;
    rng_init.seed = 0x12345678;
    qmx_hal_rng_init(&rng_init);

    QMX_HAL_ENABLE_PERIPHERAL_IRQ(PHY0_IRQ, 0x1);
    qmx_hal_register_irq_handler(PHY0_IRQ, PHY0_IRQ_Handler);
...
#if 0
    QMX_HAL_ENABLE_PERIPHERAL_IRQ(PHY1_IRQ, 0x1);
    qmx_hal_register_irq_handler(PHY1_IRQ, PHY1_IRQ_Handler);
    qmx_phy1_irq_enable(PHY_IRQ_ALL_MASK);
#endif
    // qmx_rf_set_pmu_lvd_crvt(RF_LVD_2P0);
    // qmx_rf_set_pmu_lvd_enable(true);
    // qmx_rf_irq_clr_sta(RF_IRQ_ALL_MASK);

    // qmx_hal_register_irq_handler(RFFE_IRQ, qmx_rffe_irq_handler);
    // qmx_rf_irq_enable(RF_IRQ_LVD_MASK);
    // QMX_HAL_ENABLE_PERIPHERAL_IRQ(RFFE_IRQ, 0x3);

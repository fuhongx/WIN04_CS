...
{
    qmx_hal_gpio_set_iomux(SUBG_CS_PIN, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(SUBG_CLK_PIN, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(SUBG_MOSI_PIN, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_iomux(SUBG_MISO_PIN, HAL_IOMUX_MODE2);
    qmx_hal_gpio_set_mode(SUBG_CS_PIN, HAL_GPIO_PULL_UP);
    qmx_hal_gpio_set_mode(SUBG_CLK_PIN, HAL_GPIO_PULL_UP);
    qmx_hal_gpio_set_mode(SUBG_MOSI_PIN, HAL_GPIO_PULL_UP);
    qmx_hal_gpio_set_mode(SUBG_MISO_PIN, HAL_GPIO_PULL_UP);
...
void rf_subg_write32_cmd(uint16_t u16Addr, uint8_t *pu8Buffer, uint16_t u16Len)
{
    ...
    qmx_hal_spi_set_sw_cs(SUBG_SPI_HANDLE, HAL_SPI_CS_LO);
    qmx_hal_spi_master_trx(SUBG_SPI_HANDLE, au8TxBuffer, au8SpiRxBuffer, u16Len+3, HAL_SPI_TIMEOUT_US);
    qmx_hal_spi_set_sw_cs(SUBG_SPI_HANDLE, HAL_SPI_CS_HI);
}
...
void rf_subg_read32_with_dummy(uint16_t u16Addr, uint8_t *pu8Buffer, uint16_t u16Len)
{
    ...
    qmx_hal_spi_set_sw_cs(SUBG_SPI_HANDLE, HAL_SPI_CS_LO);
    qmx_hal_spi_master_trx(SUBG_SPI_HANDLE, au8TxBuffer, au8SpiRxBuffer, u16Len+4, HAL_SPI_TIMEOUT_US);
    qmx_hal_spi_set_sw_cs(SUBG_SPI_HANDLE, HAL_SPI_CS_HI);
    for(int i = 0; i < u16Len; i++)
    {
...
void rf_test_uart_init(void)
{
    ...
    //GPIO INIT
    qmx_hal_gpio_set_iomux(RF_TEST_UART_TX_PIN, HAL_IOMUX_MODE0);
    qmx_hal_gpio_set_iomux(RF_TEST_UART_RX_PIN, HAL_IOMUX_MODE0);
    ...
    qmx_hal_uart_init(RF_TEST_UART_HANDLE, &config);
}

void rf_test_uart_tx(uint8_t *pu8Buffer, uint16_t u16Len)
{
    qmx_hal_uart_send_data(RF_TEST_UART_HANDLE, pu8Buffer, u16Len, HAL_UART_TIMEOUT_US);
}

bool bNoRet = false;
extern stUartHandle_t *qmx_hal_uart_get_handle(hal_uart_id_e uart);
uint8_t rf_test_uart_get_char(void)
{
    uint16_t u16Sta = 0;
    uint8_t rx_data;
    uint32_t u32Timeout = 0;
    stUartHandle_t *uartHandle = qmx_hal_uart_get_handle(DEBUG_UART_HANDLE);
    stUartHandle_t *rf_test_uartHandle = qmx_hal_uart_get_handle(RF_TEST_UART_HANDLE);
    while(1)
    {
        u16Sta = qmx_hal_uart_get_irq_status(RF_TEST_UART_HANDLE);
...
{
    qmx_hal_uart_receive_data(RF_TEST_UART_HANDLE, pu8Buffer, (uint32_t *)pu16Len, HAL_UART_TIMEOUT_US);
}

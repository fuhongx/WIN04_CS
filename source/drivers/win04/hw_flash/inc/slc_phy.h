...
#ifndef __SLC_PHY_H__
#define __SLC_PHY_H__
...
/* phy配置接口 */
void slc_set_phy_mode(phy_mode_e mode);
void slc_set_phy_tx_mode(phy_cfg_tx_mode_e mode);
float slc_phy_get_bw_hz(uint32_t bw);
void slc_set_monitor_reg_clr(void);
void slc_phy_set_netid(uint16_t netid);
void slc_phy_sw_reset(void);
void slc_rf_dfe_reset(void);
void slc_phy_disable_rx(void);
void slc_phy_set_trx(phy_cfg_trx_e trx_en);
void slc_phy_config_common(phy_cfg_t *cfg);
void slc_phy_cfg_done(bool done);
void slc_phy_tx_trigger_single(void);
void slc_phy_tx_trigger_repeat(void);
void slc_phy_set_sf_reduce_en(bool hsf_en);
void slc_phy_set_timeout(uint32_t timeout);
void slc_phy_set_tx_gain(phy_cfg_tx_mode_e tx_mode);
void slc_phy_set_rx_param_default(uint16_t sf);
void slc_phy_cfg_fc(uint32_t freq);
void slc_phy_set_agc_bypass_en(bool agc_bypass);

/* phy trx数据读写接口 */
void slc_phy_write_tx_buffer(uint8_t *data, uint32_t len);
void slc_phy_read_rx_buffer(uint8_t *data, uint32_t len);

// 中断类接口 */
void slc_phy0_irq_enable(uint32_t irq_mask);
void slc_phy0_irq_disable(uint32_t irq_mask);
void slc_phy1_irq_enable(uint32_t irq_mask);
void slc_phy1_irq_disable(uint32_t irq_mask);
uint32_t slc_phy_irq_get_status(void);
void slc_phy_irq_clr_status(uint32_t irq_flag);

/* 维测类接口 */
uint32_t slc_phy_monitor_get_rx_det_sta(void);

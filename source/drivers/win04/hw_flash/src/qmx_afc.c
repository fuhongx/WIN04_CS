...
void qmx_afc_set_farrow(afc_farrow_type_e farrow_type)
{
...
void qmx_afc_tx_farrow_enable(bool enable)
{
...
void qmx_afc_cfo_enable(bool enable)
{
    if (enable)
...
{
    phy_trx_cfg_param_t *trx_cfg = qmx_phy_get_trx_config_param();
    ...
    if (qmx_afc_get_sw_flag() == false) {
        if (trx_type == PHY_TX_EN) {
            qmx_afc_set_farrow(QMX_MAC_PARAM);
            qmx_afc_tx_farrow_enable(true);
        } else {
            qmx_afc_cfo_enable(false);
            qmx_afc_set_farrow(QMX_PHY_MAC_PARAM);
        }
    ...
    qmx_afc_cfo_enable(false);
    qmx_afc_tx_farrow_enable(false);
    ...
    if (fabsf(ppm) >= PPM_THRESHOLD) {
        ...
        if (qmx_rf_subg_afc_cali() != EN_ERROR_STA_OK) {
...
#else
        trx_cfg->freq = freq;
        ret += qmx_afc_cali(freq, ((trx_cfg->bw == PHY_BW_500K) ? -500000 : -250000),

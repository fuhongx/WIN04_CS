...
void qmx_lpwr_ctrl_init(qmx_lpwr_ctrl_cfg *lp_cfg)
{
    ...
    qmx_hal_pmu_set_lp_wakeup_source(lp_cfg->lp_wakeup_src_msk);
...
void qmx_lpwr_ctrl_sw_deinit(void)
{
...
uint8_t qmx_check_boot_status(void)
{
    ...
    standby_wakeup = qmx_hal_pmu_get_boot_flag();
    if (standby_wakeup != 0) {
...
__RETENTION_FUNC void qmx_lpwr_ctrl_sleep(hal_pmu_lp_mode lp_mode)
{
    ...
    boot_lpwr_mode = qmx_hal_pmu_get_boot_flag();
    if (boot_lpwr_mode) {
        qmx_hal_pmu_set_boot_flag(0);
        goto standby_wakeup;
    }
    ...
    switch (lp_mode)
    {
        ...
        case HAL_PMU_LP_MODE_STOP:
        {
            ...
            qmx_hal_pmu_phy_power_enable(g_phy_power_enable);
            qmx_hal_pmu_rf_lp_bypass(g_rf_power_enable);

            // TODO: save cali data before stop mode
            qmx_hal_pmu_set_lp_mode(HAL_PMU_LP_MODE_STOP);
            qmx_hal_sysctrl_disable_ahb_apb();
        ...
        case HAL_PMU_LP_MODE_STANDBY:
        {
            ...
            qmx_hal_pmu_set_lp_mode(HAL_PMU_LP_MODE_STANDBY);
            qmx_hal_sysctrl_disable_ahb_apb();
            qmx_hal_pmu_lpio_enable(true);

            if (g_flash_force_on == true) {
                qmx_hal_pmu_force_flash_on(true);
                rom_hw_flash_deep_power_down();
            } else {
                qmx_hal_pmu_force_flash_on(false);
                rom_hw_flash_deep_power_down();
            }
...
standby_wakeup:
        ...
        {
            qmx_hal_pmu_phy_power_enable(false);
            qmx_hal_pmu_rf_lp_bypass(false);
            qmx_hal_pmu_force_flash_on(false);
            qmx_hal_pmu_set_lp_mode(HAL_PMU_LP_MODE_SHUTDOWN);
            qmx_hal_pmu_powerdown();
            rom_hw_flash_deep_power_down();

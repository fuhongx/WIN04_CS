...
void slc_private_spi_init(void)
{
    ...
    slc_hal_gpio_set_iomux(SLC_PRI_CS_PIN, SLC_PRI_SPI_IOMUX);
    slc_hal_gpio_set_iomux(SLC_PRI_CLK_PIN, SLC_PRI_SPI_IOMUX);
    slc_hal_gpio_set_iomux(SLC_PRI_MOSI_PIN, SLC_PRI_SPI_IOMUX);
    slc_hal_gpio_set_iomux(SLC_PRI_MISO_PIN, SLC_PRI_SPI_IOMUX);
    slc_hal_gpio_set_mode(SLC_PRI_CS_PIN, HAL_GPIO_PULL_UP);
    slc_hal_gpio_set_mode(SLC_PRI_CLK_PIN, HAL_GPIO_PULL_UP);
    slc_hal_gpio_set_mode(SLC_PRI_MOSI_PIN, HAL_GPIO_PULL_UP);
    slc_hal_gpio_set_mode(SLC_PRI_MISO_PIN, HAL_GPIO_PULL_UP);

    slc_hal_sysctrl_peripheral_clk_enable(SLC_PRI_SPI_GATE, true);
    slc_hal_sysctrl_peripheral_mod_reset(SLC_PRI_SPI_GATE);
    ...
    slc_hal_spi_init(SLC_PRI_SPI, &config);
}
...
void slc_rf_spi_write32_cmd(uint16_t addr, uint32_t data)
{
    ...
    slc_hal_spi_set_sw_cs(SLC_PRI_SPI, HAL_SPI_CS_LO);
    slc_hal_spi_master_trx(SLC_PRI_SPI, tx_buf, rx_buf, 7, HAL_SPI_TIMEOUT_US);
    slc_hal_spi_set_sw_cs(SLC_PRI_SPI, HAL_SPI_CS_HI);
}
...
uint32_t slc_rf_spi_read32_cmd_with_dummy(uint16_t addr)
{
    uint8_t tx_buf[8] = {0};
    uint8_t rx_buf[8] = {0};
    uint32_t rddata = 0;
    ...
    // SUBG default is big endian
    tx_buf[0] = SLC_RF_DUMMY;
    tx_buf[1] = (addr >> 8) & 0xFF;
    tx_buf[2]  = addr & 0xFF;
    tx_buf[3] = 0;

    slc_hal_spi_set_sw_cs(SLC_PRI_SPI, HAL_SPI_CS_LO);
    slc_hal_spi_master_trx(SLC_PRI_SPI, tx_buf, rx_buf, 8, HAL_SPI_TIMEOUT_US);
    slc_hal_spi_set_sw_cs(SLC_PRI_SPI, HAL_SPI_CS_HI);
...
{
    uint32_t mask = ((1U << (end_bit - start_bit + 1U)) - 1U) << start_bit;
    uint32_t reg = slc_phy_spi_read32_cmd(addr);
    reg = (reg & ~mask) | ((val << start_bit) & mask);
    slc_phy_spi_write32_cmd(addr, reg);
}
